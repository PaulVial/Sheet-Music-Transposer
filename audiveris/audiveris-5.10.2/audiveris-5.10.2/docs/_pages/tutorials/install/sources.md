...
This will create a sub-directory named "audiveris" in the current directory and populate it with
...
Once in the `audiveris` project directory, we can select which branch we want.
...
application, then we don't need to go through gradle for each and every run.

Once we have built the software with the gradle `build` command as stated above,
